...
| Output a report | report | An MD file exists under `_Prompt\02_buildai\` (legacy `_Prompt\01_FromBuilderAi\` also accepted) + **"Readers and Purpose" section at the top of the report** |
| Output a report / dev log | report | **Two-tier structure**: judgment-centric content goes to the upstream doc repo as a canonical report; fact-centric diffs/commit-unit content goes to the working repo as a dev log. When in doubt, write both |
